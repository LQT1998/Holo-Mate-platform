...
@pytest.mark.asyncio
async def test_device_pairing_flow(streaming_client: AsyncClient, db_session: Session, authenticated_user_headers: dict):
    """
    ...
    list_response = await streaming_client.get("/devices", headers=authenticated_user_headers)
    ...
    update_response = await streaming_client.put(f"/devices/{device_id}", json=update_payload, headers=authenticated_user_headers)
